= Explore environment and router configuration
...
TIP: Your first run will take a moment to spin up.
...
Refer to the `commands` output at the end (scroll right in the box above) to see that the resource module was able to figure out which commands needed to be run to accomplish the configuration, even though our configuration data is stored as YAML. We were only missing the `timers` part. This is for the lab so that BGP can more quickly tell when something is wrong, and we'll be making use of that later.
...
NOTE: We are doing this work on a router and working with routing protocols, but as you work through the lab, consider how this could also apply to switches. The way we are working with interfaces in this lab would be no different.
