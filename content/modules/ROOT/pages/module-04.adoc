...
* The second task: Creates push subscriptions to all the YANG nodes described in the `telemetry_nodes` variable. The line `update-policy periodic 3000` is counting in **centiseconds**, so we'll expect a message every 30 seconds.
